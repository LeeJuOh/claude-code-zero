...
Bookmarks are stored in `~/.claude-code-zero/plugin-bookmarks/data/wishlist.json`. Each entry includes `name`, `url`, `description`, and an optional `installHint` field with the install command. Data is stored outside the plugin directory to persist across plugin updates.
