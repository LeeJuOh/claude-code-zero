...
Navigation pattern for multi-section reports. Provides a sticky sidebar TOC on desktop and a sticky horizontal scrollable bar on mobile. Required for reports with 4+ sections.

## Layout Structure

Two-column CSS Grid: sidebar (TOC) + main content. Collapses to single-column on mobile.
...
Place before `</body>`, after any Mermaid initialization:
...
- The `rootMargin: '-10% 0px -80% 0px'` means a section is "active" when its heading enters the top 10-20% of the viewport
- `grid-template-columns: 170px` works for most TOCs. Go up to `200px` for longer section names
- For reports with fewer than 4 sections, skip the TOC — it adds clutter without value
- On mobile, the active tab auto-scrolls into the center of the bar as the user scrolls
