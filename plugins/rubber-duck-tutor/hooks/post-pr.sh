...
# The context below hands the model both paths; without them it can't run the scripts.
[[ -n "${CLAUDE_PLUGIN_ROOT:-}" && -n "${CLAUDE_PLUGIN_DATA:-}" ]] || exit 0
SCRIPTS="${CLAUDE_PLUGIN_ROOT}/skills/ducking/scripts"

duck__check_rate_limit "ship" 1

STREAK=$(bash "$SCRIPTS/ignore-streak.sh" --data-dir "$CLAUDE_PLUGIN_DATA" 2>/dev/null)
[[ "$STREAK" =~ ^[0-9]+$ ]] || STREAK=0
