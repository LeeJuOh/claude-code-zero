...
Reads the workspace directory, discovers runs, embeds output data into
a self-contained HTML page, and serves via HTTP or writes a static file.
...
def get_mime_type(path: Path) -> str:
    overrides = {".svg": "image/svg+xml", ".xlsx": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}
...
def find_runs(workspace: Path) -> list[dict]:
    ...
    runs: list[dict] = []
    _find_runs(workspace, workspace, runs)
    runs.sort(key=lambda r: (r.get("eval_id", float("inf")), r["id"]))
    return runs


def _find_runs(root: Path, current: Path, runs: list[dict]) -> None:
...
def _build_run(root: Path, run_dir: Path) -> dict | None:
    ...
    return {"id": run_id, "prompt": prompt or "(No prompt found)", "eval_id": eval_id, "outputs": output_files, "grading": grading}
...
def _embed_file(path: Path) -> dict:
    ...
    if ext in TEXT_EXTENSIONS:
        ...
        try:
            ...
            return {"name": path.name, "type": "error", "content": "(Error)"}
...
def load_previous_iteration(workspace: Path) -> dict[str, dict]:
    ...
    if feedback_path.exists():
        try:
            ...
            feedback_map = {r["run_id"]: r["feedback"] for r in fb_data.get("reviews", []) if r.get("feedback", "").strip()}
...
def generate_html(runs: list[dict], skill_name: str, previous: dict[str, dict] | None = None, benchmark: dict | None = None) -> str:
    ...
    if previous:
        for run_id, d in previous.items():
            if d.get("feedback"):
                previous_feedback[run_id] = d["feedback"]
            if d.get("outputs"):
                previous_outputs[run_id] = d["outputs"]

    embedded = {"skill_name": skill_name, "runs": runs, "previous_feedback": previous_feedback, "previous_outputs": previous_outputs}
    if benchmark:
...
class ReviewHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            fb = self.feedback_path.read_bytes() if self.feedback_path.exists() else b"{}"
            self.send_response(200)
    ...
    def do_POST(self):
        if self.path == "/api/feedback":
            ...
            try:
                fb_data = json.loads(body)
                self.feedback_path.write_text(json.dumps(fb_data, indent=2) + "\n")
                resp = b'{"ok":true}'
                self.send_response(200)
...
def main():
    ...
    parser.add_argument("--skill-name", "-n", type=str, default=None)
