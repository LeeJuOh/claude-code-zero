...
and benchmark.md with pass_rate, time, and tokens for each configuration.
...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    - Workspace: <dir>/eval-N/<config>/run-N/grading.json
    - Legacy:    <dir>/runs/eval-N/<config>/run-N/grading.json
    """
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines = [
        ...
        "|--------|------------|---------------|-------|",
        f"| Pass Rate | {a.get('pass_rate', {}).get('mean', 0)*100:.0f}% | {b.get('pass_rate', {}).get('mean', 0)*100:.0f}% | {delta.get('pass_rate', '-')} |",
        f"| Time | {a.get('time_seconds', {}).get('mean', 0):.1f}s | {b.get('time_seconds', {}).get('mean', 0):.1f}s | {delta.get('time_seconds', '-')}s |",
        f"| Tokens | {a.get('tokens', {}).get('mean', 0):.0f} | {b.get('tokens', {}).get('mean', 0):.0f} | {delta.get('tokens', '-')} |",
    ]
