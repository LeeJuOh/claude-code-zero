...
전/후 그림(사실만, 판단 금지, 무변경 시 생략). Quiz 규칙(5문항·중난도·함정 금지·어절 수 ±1·클릭
...
fact sheet/게이트/발행/사이드카/PNG 자가점검) 절차 무변경 — 단 Intent Check 질문 교체, fact sheet 출처 범위를 탐색한 소스 file:line까지 확장, 자가점검 체크리스트 어휘를 4섹션으로 교체(D8). Code 조각 예산 3~8개·≤150행(D4).
...
블록(`<details>` 또는 구분선 아래). 300행 캡 유지 — 초과 시 부록 → 깊은 배경 → Code 조각 순으로 절삭, Intuition·Quiz는 유지(D8). 번역 규칙(산문 번역, 코드·경로 원문) 유지.
...
### S3 — description·README·버전 정리 (스토리 18, 19; 결정 D10, D11)
